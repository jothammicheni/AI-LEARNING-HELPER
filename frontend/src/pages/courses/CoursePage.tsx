    ...
    if (isAudioMode) {
      ...
      try {
        const response = await axios.post(
          `http://localhost:5007/api/chapter/speak/${selectedChapter.id}`
        );
        setAudioUrl(`http://localhost:5007${response.data.audioUrl}`);
        setIsAudioMode(true);
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="ml-64 flex-1 p-8">
        {selectedChapter && (
          <div>
          <h1 className="text-3xl font-bold mb-4 text-teal-700">{`${course}`}</h1>
